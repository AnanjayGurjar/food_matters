...
exports.signupUser = BigPromise(async (req, res, next) => {
	...
	const { userId, name, phoneNumber, email, addressString, longitude, latitude, userType, documentId, photo, fcmToken } = req.body;
	...
	if (!email || !longitude || !latitude || !name || !phoneNumber || !userType) {
		return res.status(401).json({
			status: 401,
			message: "Please provide all required details including name, phoneNumber, email and address, UserType"
		});
	}
	...
	if (userType == "Volunteer") {
		punyaPoint = initializePunyaPoint(user._id, name);
		console.log("punya point initialized ", punyaPoint);
	}
...
async function initializePunyaPoint(userId, name) {

	const punyaPoint = await PunyaPoint.create({
		user: userId,
		userName: name
	});
